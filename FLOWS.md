...
5. **Switch to Map view** — day cards STAY on the left
   ...
   - Mobile: ⚠️ NEEDS FIX — should become bottom sheet

6. **Add activities** from three entry points:
   - **6a. Map click**: Click any POI → InfoWindow shows "📍 Activity" or "🏨 Hotel"
   - **6b. Search bar**: ⚠️ NOT YET BUILT — planned for map top-center
   - **6c. AI Discovery Radar**: Drag pointer, pick category, Ghost Pins appear
...
10. **Add flight/transit information**
    - ⚠️ NOT YET BUILT
    - PLAN: Logistics drawer inside each day card
    - Data lives in existing `transit[]` array with `autoGenerated: false`
